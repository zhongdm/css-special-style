...
function drawNeedle () {
  ...
  for (let r = 0; r < 0; r++) {
    ...
    let beginPoint = {
      x: startPoint.x + Math.sin(angle) * radius,
      y: startPoint.y + (radius - Math.cos(angle) * radius),
      ex: center.x + Math.cos(angle) * (radius - maxNeedle),
      ey: center.y - Math.sin(angle) * (radius - maxNeedle)
    }
