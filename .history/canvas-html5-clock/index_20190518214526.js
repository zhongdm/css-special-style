...
function run () {
  ...
  speed = (second - 15) * 6 * Math.PI / 180
  console.log( second, speed / 60 + (minute - 15) * 6 * Math.PI / 180)
  // drawHourNeedle()
  drawMinuteNeedle( speed / 60 + (minute - 15) * 6 * Math.PI / 180)
  drawSecondNeedle(speed)
  // speed += 6
}
...
function drawMinuteNeedle (angle) {
  ...
  let minutePassPoint = {
    x: center.x + Math.cos(angle) * mLen,
    y: center.y + Math.sin(angle) * mLen
  }
