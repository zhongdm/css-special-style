...
function run () {
  ...
  console.log(needlePosition(minute))
  drawHourNeedle(needlePosition(second) / 60 / 12 + needlePosition(minute) / 12 + 30 * Math.PI / 180 * (hour - 3))
...
function drawLine (r, baseAngle, endRadius, textRadius) {
  ...
  let beginPoint = {
    ...
    ty: textRadius ? center.y + Math.sin(angle) * textRadius + 4 : 0
  }
  ctx.beginPath()
